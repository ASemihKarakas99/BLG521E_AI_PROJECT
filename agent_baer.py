...
from buffers.balanced_attentive_experience_replay import BalancedAttentiveReplayBuffer
...
class Agent:
    ...
    def run(self, num_episodes=None, render=False): 
        ...
        if self.buffer_type == 'uniform':
        ...
        elif self.buffer_type == 'baer':
            memory = BalancedAttentiveReplayBuffer(
            ...
            # memory = BalancedAttentiveReplayBuffer(num_states, 1, self.replay_memory_size, policy_dqn)
        ...
        for episode in range(num_episodes):
            ...
            if memory.real_size > self.mini_batch_size:
                if self.buffer_type == 'uniform':
                    ...
                    batch, is_weights = memory.sample(self.mini_batch_size, current_state=state, embedding_model=policy_dqn)
                    # batch, is_weights = memory.sample(self.mini_batch_size, current_state=state)
                    self.optimize_baer(batch, is_weights, policy_dqn, target_dqn)
    ...
    def optimize_baer(self, batch, is_weights, policy_dqn, target_dqn):
        states, actions, rewards, next_states, terminations = batch

        # Ensure actions are [B, 1] for gather
        actions = actions.view(-1).long().unsqueeze(1)
